#ifndef gst_analyzer_h
#define gst_analyzer_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   gst_analyzer(TString in, TString out, TTree *tree=0);
   virtual ~gst_analyzer();
...
#ifdef gst_analyzer_cxx
gst_analyzer::gst_analyzer(TString InputFile, TString OutputFile, TTree *tree) : fChain(0) 
{
...
gst_analyzer::~gst_analyzer()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t gst_analyzer::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t gst_analyzer::LoadTree(Long64_t entry)
{
...
void gst_analyzer::Init(TTree *tree)
{
...
Bool_t gst_analyzer::Notify()
{

   return kTRUE;
}

void gst_analyzer::Show(Long64_t entry)
{
...
Int_t gst_analyzer::Cut(Long64_t entry)
{
